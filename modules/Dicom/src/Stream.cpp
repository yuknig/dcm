...
size_t StreamRead::read(void* a_dest, size_t a_size_in_bytes)
{
    assert(m_impl);
...
{
    using ImplT = StreamReadImplT<StreamRead::Impl, FileStreamRead>;
    std::unique_ptr<ImplT> impl(new ImplT(a_filename));
