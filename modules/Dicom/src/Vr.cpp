...
VRType vrCodeToVrType(const VRCode a_vrCode) {
    ...
    static const std::array<std::pair<VRCode, VRType>, 32> SortedVRs = {{
    ...
    assert(std::is_sorted(SortedVRs.begin(), SortedVRs.end(),
        [](const auto& a_lhs, const auto& a_rhs) {
        return a_lhs.first < a_rhs.first;
    }));
    static_assert(SortedVRs.size() == static_cast<size_t>(VRType::Max) + 1, "VRCode / VRType mismatch");

    const auto findIt = std::lower_bound(SortedVRs.begin(), SortedVRs.end(), a_vrCode,
        [](const auto& a_lhs, const auto& a_rhs) {
